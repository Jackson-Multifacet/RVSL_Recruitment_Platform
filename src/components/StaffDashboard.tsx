...
import { 
  ...
  BarChart3, PieChart as PieChartIcon, Activity, User
} from 'lucide-react';
import { 
...
export function StaffDashboard({ user }: { user: any }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        handleFirestoreError(error, OperationType.GET, 'staff_dashboard');
      }
    };
    fetchData();
  }, []);
          ...
          await addDoc(collection(db, 'mail'), {
            ...
            message: {
              ...
              html: `
                ...
                <p>Dear ${candidate.fullName || candidate.name?.firstName || 'Candidate'},</p>
                <p>Your application status for the position of <strong>${job.title}</strong> at ${job.company} has been updated to: <strong>${newStatus}</strong>.</p>
                <p>Please log in to your dashboard for more details or to communicate with your agent.</p>
                <br/>
                <p>Best regards,</p>
                <p>The RVSL Team</p>
              `
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-12">
      ...
      {/* Other tabs (Jobs, Updates) would follow similar patterns */}
