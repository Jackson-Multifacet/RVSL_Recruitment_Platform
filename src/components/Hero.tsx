...
export function Hero({ onLogin }: { onLogin: () => void }) {
  return (
    <header className="max-w-7xl mx-auto px-4 py-16 md:py-24 flex flex-col items-center text-center">
      <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-orange-50 dark:bg-orange-900/20 text-orange-700 dark:text-orange-400 text-xs font-bold uppercase tracking-wider mb-6">
        <Briefcase aria-hidden="true" className="w-3 h-3" />
      ...
      <div className="flex flex-col sm:flex-row gap-4 w-full sm:w-auto">
        <button
          type="button"
          onClick={onLogin}
          className="w-full sm:w-auto px-6 sm:px-12 py-3 sm:py-4 bg-orange-600 text-white rounded-2xl font-bold shadow-xl hover:bg-orange-700 transition-all transform hover:-translate-y-1 flex items-center justify-center gap-2 focus:outline-none focus:ring-2 focus:ring-orange-500"
        >
          Get Started Now <ChevronRight aria-hidden="true" className="w-5 h-5" />
        </button>
      </div>
    </header>
  );
